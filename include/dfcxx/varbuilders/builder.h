...
public:
  DFVariable *
  buildStream(const std::string &name, Direction direction, GraphHelper &helper,
              const DFType &type);

  DFVariable *
  buildScalar(const std::string &name, Direction direction, GraphHelper &helper,
              const DFType &type);

  DFVariable *
  buildConstant(GraphHelper &helper, const DFType &type, TypeKind kind,
                ConstantValue value);
};
